...
# NOTE 2: THIS ONE RUNS FOR R EQUAL TO 1 
...
for (run in 1:n_runs) {
  ...
  if(run <= n_grid) {
    E = round(initial_grid[run, 1]*diff(range(I_range)) + min(I_range))
    I = round(initial_grid[run, 2]*diff(range(E_range)) + min(E_range))
    R = round(initial_grid[run, 3]*diff(range(R_range)) + min(R_range))
  } else {
  ...
  if(run >= n_grid) {
    ...
    big_grid = matrix(runif(200*3), ncol = 3)
    pred_vals = predict(GPmodel, big_grid)
    # Find the biggest MSE
    big_mse = which.max(pred_vals$MSE)
    inputs = big_grid[big_mse,]
    new_input = c(
      round(inputs[1]*diff(range(E_range)) + min(E_range)),
      round(inputs[2]*diff(range(I_range)) + min(I_range)),
      round(inputs[3]*diff(range(R_range)) + min(R_range))
    )
