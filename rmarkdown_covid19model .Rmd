...
### Get CFR
### This is the probability of dying given infection for different age categories. 
...
### Plot Fig 6 in paper supplemental
...
The goal below is the get `f` which in the paper is denoted $\pi_{s,m}$. This is the weight used in the calculation of the expected number of deaths. $\pi_{s,m}$ is derived from $\pi_m$ (labeled as `convolution` in the code) where
...
```{r standata2, eval = FALSE}
  ...
  if(DEBUG) { # OLD -- but faster for testing this part of the code
    ...
    ### This is pi_m in the paper which is defined as "the infection to death distribution" 
