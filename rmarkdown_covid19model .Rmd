...
The goal below is the get `f` which in the paper is denoted $\pi_{s,m}$; the "infection-to-death distribution", on a given day $s$, for country, $m$. This is the weight used in the calculation of the expected number of deaths. $\pi_{s,m}$ is derived from $\pi_m$ (labeled as `convolution` in the code) where

$\pi_m \sim CFR_m*(\Gamma(5.1,0.86) + \Gamma(18.8,0.45))$,

where $CFR_m$ is the probability of dying given infection (pre-calculated and read in above).
